...
- [x] Do 13 subtasks from 1 to 13 and add screenshots of each:
    ...
    12. [x] بخش متناظر با `core-y` در `Makefile` من در پوشه‌ی کد منبع وجود نداشت که علت آن تفاوت نسخه‌ی کرنل (۶) با نسخه‌ی مورد نظر صورت سوال است. در عوض، در فایل `Kbuild` بخش `obj-y` را باید آپدیت کرد. به انتهای این فایل `hello.o` (و در ادامه `adder.o`) را اضافه کردم.
    ![image](images/28.png)

    13. [x] این قسمت را در فایل ۶۴ بیتی عوض کردم و با توجه به داکیومنت‌ها، به جای `i386` از `common` استفاده کردم. بعدا `adder` را هم اضافه کردم:
