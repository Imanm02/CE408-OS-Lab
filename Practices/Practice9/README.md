...
نکته‌ای که موقع مرتب کردن این گزارش و با نگاه دوباره به خروجی‌ها متوجه شدیم: `generic_handle_irq` را از context پردازه صدا زده‌ایم، یعنی از داخل `write` روی فایل proc، و کرنل ۶.۸ این کار را قبول نمی‌کند. به همین دلیل در خروجی‌های `dmesg` که در ادامه آمده یک trace هشدار دیده می‌شود و پیام «My Software Interrupt Handler Invoked!» هیچ‌وقت چاپ نشده است. پس در عمل ثبت و آزادسازی IRQ و مسیر سمت کاربر کار کرده‌اند ولی خود هندلر اجرا نشده است. کد را همان طور که تحویل دادیم نگه داشته‌ایم.
