# Practice 8: Kernel modules and syscall hooking
...
نکته: در حلقه‌ی این کد اندیس آرایه از ۰ شروع می‌شود ولی `index + 1` را چاپ کرده‌ایم. پس عددی که در خروجی و در اسکرین‌شات‌ها می‌بینید یک واحد بیشتر از شماره‌ی واقعی فراخوانی سیستمی است، چون در x86-64 شماره‌ها از ۰ شروع می‌شوند. آدرس‌ها درست هستند و فقط همین شماره جابه‌جاست.
...
نکته‌ای که موقع مرتب کردن این گزارش متوجه شدیم: شرطی که در `modified_getdents` نوشته‌ایم شماره‌ی inode هر رکورد را با inode خود `/home` مقایسه می‌کند، و نتیجه‌اش این است که خود `/home` دست‌نخورده می‌ماند و در عوض لیست دایرکتوری‌های دیگر از یک جایی به بعد بریده می‌شود. ضمناً در کد هیچ `copy_to_user`ای نیست، پس بافر کاربر بازنویسی نمی‌شود و تنها چیزی که تغییر می‌کند مقدار برگشتی فراخوانی است. کد را همان طور که تحویل دادیم نگه داشته‌ایم.

بخش دوم صورت آزمایش (این که بدافزارها یا ابزارهای ضد بدافزار از هوک کردن چه بهره‌ای می‌برند) را در گزارش اصلی جواب نداده‌ایم.
